...
export default {
    ...
    props: {
        ...
        // isFormData = false时需要传入当前 value 否则会通过 curNodePathArr 自动计算
        curValue: {
            type: null,
            default: 0
        },
    ...
    computed: {
        value: {
            ...
            set(value) {
                ...
                if (this.isFormData) {
                    setPathVal(this.rootFormData, this.curNodePathArr, trueValue);
                }
    ...
    render(h) {
        ...
        // 判断是否为根节点
        const isRootNode = this.curNodePathArr.length === 0;
            ...
            'el-form-item',
            {
                ...
                props: {
                    ...
                    ...this.isFormData ? {
                        ...
                        prop: isRootNode ? '__$$root' : pathArray2prop(self.curNodePathArr),
                        rules: [
                            {
                                    ...
                                    if (self.required) {
                                        if (isEmpty) {
                                            const requireErrObj = {
                                                keyword: 'required',
                                                params: {
                                                    missingProperty: self.curNodePathArr.map(item => `[${item}]`).join('')
                                                }
                                            };
